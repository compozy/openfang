DROP TABLE IF EXISTS workflow_signal__task17_old;
DROP TABLE IF EXISTS workflow_checkpoint__task17_old;
DROP TABLE IF EXISTS workflow_run__task17_old;
...
ALTER TABLE workflow_signal RENAME TO workflow_signal__task17_old;
ALTER TABLE workflow_checkpoint RENAME TO workflow_checkpoint__task17_old;
ALTER TABLE workflow_run RENAME TO workflow_run__task17_old;
...
SELECT
    ...
    data_json,
    created_at
FROM workflow_checkpoint__task17_old
WHERE EXISTS (
    SELECT 1
    FROM workflow_run
    WHERE workflow_run.run_id = workflow_checkpoint__task17_old.run_id
);
...
SELECT
    ...
    created_at,
    consumed_at
FROM workflow_signal__task17_old
WHERE EXISTS (
    SELECT 1
    FROM workflow_run
    WHERE workflow_run.run_id = workflow_signal__task17_old.run_id
);
...
DROP TABLE workflow_signal__task17_old;
DROP TABLE workflow_checkpoint__task17_old;
DROP TABLE workflow_run__task17_old;
