DROP TABLE IF EXISTS workflow_signal__task19_old;
DROP TABLE IF EXISTS workflow_checkpoint__task19_old;
DROP TABLE IF EXISTS workflow_run__task19_old;
...
DROP INDEX IF EXISTS idx_workflow_signal_run_idempotency;
...
ALTER TABLE workflow_signal RENAME TO workflow_signal__task19_old;
ALTER TABLE workflow_checkpoint RENAME TO workflow_checkpoint__task19_old;
ALTER TABLE workflow_run RENAME TO workflow_run__task19_old;
...
SELECT
    ...
    workflow_version,
    CASE status
        WHEN 'waiting' THEN 'waiting_signal'
...
SELECT
    ...
    data_json,
    created_at
FROM workflow_checkpoint__task19_old
WHERE EXISTS (
    SELECT 1
    FROM workflow_run
    WHERE workflow_run.run_id = workflow_checkpoint__task19_old.run_id
);
...
SELECT
    ...
    created_at,
    consumed_at
FROM workflow_signal__task19_old
WHERE EXISTS (
    SELECT 1
    FROM workflow_run
    WHERE workflow_run.run_id = workflow_signal__task19_old.run_id
);
...
CREATE UNIQUE INDEX idx_workflow_signal_run_idempotency
    ON workflow_signal(run_id, idempotency_key);

DROP TABLE workflow_signal__task19_old;
DROP TABLE workflow_checkpoint__task19_old;
DROP TABLE workflow_run__task19_old;
