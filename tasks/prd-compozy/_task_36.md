...
- [x] 36.1 Implement the `TriggerMatchEngine` struct. It holds a snapshot of enabled trigger
...
- [x] 36.2 Implement action dispatch for each target kind. After matching, each resolved target
...
- [x] 36.3 Register `POST /api/v1/events` in the router. The handler accepts an event payload
...
- [x] 36.4 Register `POST /api/v1/events/dry-run` in the router. The handler runs the match
...
- [x] 36.5 Implement `max_fires` and `cooldown_secs` enforcement in the match engine. Before
...
- [x] 36.6 Ensure the match engine reloads its trigger set when triggers are created, updated,
...
- [x] 36.7 Add engine-level and integration tests. See the Tests section below.

- [x] 36.8 Confirm that `cargo fmt --all`, `cargo clippy --workspace --all-targets -- -D warnings`,
...
- [x] Match pattern with `event = "issue.created"` matches an event whose `event` field is exactly
      `"issue.created"` and does not match an event with `event = "issue.updated"`.
- [x] Match pattern with `contains = "ISSUE-123"` matches an event whose serialized payload
      contains the substring and does not match a payload without it.
- [x] Match pattern with no `event` field (wildcard) matches any event regardless of its event
      type.
- [x] `max_fires` enforcement: a trigger with `max_fires = 3` and `fire_count = 3` does not match
      even when the event matches all other conditions.
- [x] `cooldown_secs` enforcement: a trigger with `cooldown_secs = 60` and `last_fired_at` within
...
- [x] E2E event -> trigger -> action flow: create an enabled `workflow_start` trigger matching
...
- [x] A disabled trigger does not fire: create a trigger, disable it, send a matching event via
      `POST /api/v1/events`, assert `matched_triggers` is empty and no workflow run is created.
- [x] `POST /api/v1/events/dry-run` with a matching event returns `would_execute: true` and the
      correct `effects.matched_triggers` list without dispatching any action.
- [x] Multiple triggers match the same event: create two enabled triggers matching the same event
      type with different target kinds; send the event and verify both triggers fire and both
      effects are counted.
- [x] Trigger error isolation: create two triggers where the first has an invalid target (e.g.,
      references a nonexistent workflow); send a matching event and verify the second trigger still
      fires successfully.

### Regression and Anti-Pattern Guards

- [x] No event is silently dropped: `POST /api/v1/events` must match against all enabled triggers
...
- [x] No trigger fires when disabled: verified through both the matching engine unit test and the
      E2E integration test above.
- [x] Trigger matching must not be conflated with HITL or in-step interaction semantics; triggers
